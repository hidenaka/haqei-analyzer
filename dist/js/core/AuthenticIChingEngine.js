...
window.AuthenticIChingEngine = {
  ...
  async initializeInterpretationSystem() {
    this.interpretationSystem = {
      ...
      // bunenjin解釈
      bunenjin_interpretation: this.interpretBunenjin.bind(this),
  ...
  setupTransformationRules() {
    this.transformationRules = {
      // bunenjin変容
      bunenjin_transformations: new Map([
  ...
  async performDivination(query, options = {}) {
    ...
    try {
      ...
      return {
        ...
        bunenjin_integration: await this.integrateBunenjinPrinciples(divinationResult.hexagram, query)
      };
  ...
  async generateInterpretation(hexagramNumber, query, style) {
    const interpretation = {
      ...
      bunenjin: style === 'bunenjin' ? await this.interpretBunenjin(hexagramNumber, query) : null,
  ...
  // bunenjin解釈
  async interpretBunenjin(hexagramNumber, query) {
    const interpretation = {
      harmony_aspect: null,
    ...
    // 各原理からの解釈取得
    interpretation.harmony_aspect = this.bunenjinInterpretation.harmony_interpretations.get(hexagramNumber);
    interpretation.compassion_aspect = this.bunenjinInterpretation.compassion_interpretations.get(hexagramNumber);
    interpretation.wisdom_aspect = this.bunenjinInterpretation.wisdom_interpretations.get(hexagramNumber);
    interpretation.authenticity_aspect = this.bunenjinInterpretation.authenticity_interpretations.get(hexagramNumber);

    // 統合メッセージ生成
    interpretation.integrated_message = await this.generateIntegratedBunenjinMessage(
      hexagramNumber, 
      interpretation, 
      query
    );
  ...
  // 統合bunenjinメッセージ生成
  async generateIntegratedBunenjinMessage(hexagramNumber, interpretation, query) {
    const hexagramData = this.completeDatabase.hexagrams.get(hexagramNumber);
    if (!hexagramData || !hexagramData.bunenjin_aspects) {
      return 'bunenjinの原理に従って調和的に進んでください';
    }

    const aspects = hexagramData.bunenjin_aspects;
    let message = `${hexagramData.name}の智慧において、`;

    // 最も関連する側面を強調
    const queryAnalysis = await this.analyzeBunenjinRelevance(query);
    const primaryAspect = this.findPrimaryBunenjinAspect(queryAnalysis);
  ...
  // bunenjin関連性分析
  async analyzeBunenjinRelevance(query) {
    const relevance = {
      harmony: 0,
      compassion: 0,  
      wisdom: 0,
  ...
  // 主要bunenjin側面特定
  findPrimaryBunenjinAspect(relevance) {
    let maxRelevance = 0;
  ...
  // bunenjin原理統合
  async integrateBunenjinPrinciples(hexagramNumber, query) {
    const integration = {
      harmony_integration: '',
    ...
    const hexagramData = this.completeDatabase.hexagrams.get(hexagramNumber);
    if (!hexagramData || !hexagramData.bunenjin_aspects) {
      integration.overall_message = 'bunenjinの四原理を日常生活に統合して実践してください';
      return integration;
    }
    ...
    integration.overall_message = 
      `${hexagramData.name}の智慧を通じて、bunenjinの四原理（調和・慈悲・智慧・真実）を` +
  ...
  getDefaultDivination(query) {
    return {
      ...
      bunenjin_integration: {
        overall_message: 'bunenjinの原理に従って調和的な人生を歩んでください'
      }
    };
  }
};
