...
class H384DatabaseCompat {
  ...
  getLineText(hexagram, line) {
    // 入力値の検証とデフォルト値設定
    const validHexagram = parseInt(hexagram) || 1;
    const validLine = parseInt(line) || 1;
    
    // 範囲チェック
    const safeHexagram = Math.max(1, Math.min(64, validHexagram));
    const safeLine = Math.max(1, Math.min(6, validLine));
    
    const key = `${safeHexagram}-${safeLine}`;
    const yaoNames = ['初', '二', '三', '四', '五', '上'];
    const yaoName = yaoNames[safeLine - 1];
    
    return this.lineTexts.get(key) || `第${safeHexagram}卦 第${yaoName}爻`;
  }
  ...
  getSymbolText(hexagram, line) {
    ...
    return this.symbolTexts.get(key) || `第${safeHexagram}卦 第${yaoName}爻の象`;
  }
