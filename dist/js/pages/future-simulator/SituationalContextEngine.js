...
window.SituationalContextEngine = {
  ...
  // 初期化メソッド群 - Enhanced with error handling
  initializeContextFrameworks() {
    try {
      console.log('🔧 Initializing context frameworks...');
      
      this.contextFrameworks = {
        situationType: this.safeCreateFramework('createSituationTypeFramework'),
        urgency: this.safeCreateFramework('createUrgencyFramework'),
        scope: this.safeCreateFramework('createScopeFramework'),
        nature: this.safeCreateFramework('createNatureFramework'),
        adaptation: this.safeCreateFramework('createAdaptationFramework')
      };
      
      console.log('✅ Context frameworks initialized successfully');
    } catch (error) {
      console.error('❌ Context frameworks initialization error:', error);
      this.contextFrameworks = {};
    }
  },

  // Safe framework creation helper
  safeCreateFramework(methodName) {
    try {
      const method = this[methodName];
      if (typeof method === 'function') {
        const result = method.call(this);
        console.log(`✅ ${methodName} framework created`);
        return result;
      } else {
        console.warn(`⚠️ Framework method ${methodName} not available`);
        return {};
      }
    } catch (error) {
      console.error(`❌ Error creating framework ${methodName}:`, error.message);
      return {};
    }
  },
  ...
  loadContextMappings() {
    return {
      situational: {
        'decision': ['決める', '選ぶ', '判断', '決断', '選択'],
        'problem': ['問題', '困難', '課題', 'トラブル', '障害'],
        'opportunity': ['チャンス', '機会', '可能性', '好機'],
        'relationship': ['人間関係', '関係', '付き合い', 'つながり']
      },
      environmental: {
        'work': ['仕事', '職場', '会社', '業務', 'キャリア'],
        'personal': ['個人的', '私的', 'プライベート'],
        'social': ['社会的', '公的', 'コミュニティ'],
        'family': ['家庭', '家族', '家庭的']
      },
      adaptations: {
        'flexible': ['柔軟', '適応', '変化対応', '臨機応変'],
        'stable': ['安定', '一貫', '継続', '保持'],
        'innovative': ['革新', '新しい', '創造', '独创']
      }
    };
  },

  loadAdaptationMappings() {
    return {
      strategies: {
        'accommodation': ['適応', '受容', '柔軟対応', '調整'],
        'resistance': ['抵抗', '反対', '維持', '守る'],
        'transformation': ['変化', '変革', '進化', '成長'],
        'integration': ['統合', '融合', '結合', '一体化']
      },
      contexts: {
        'high_pressure': ['プレッシャー', 'ストレス', '緊張', '玉限'],
        'supportive': ['支援', 'サポート', '協力', '支え'],
        'uncertain': ['不確実', '曖昧', '不明', '不安定']
      }
    };
  },

  loadStrategyMappings() {
    return {
      haqei: {
        'harmony': ['調和', 'バランス', '均衡', '穏やか'],
        'transformation': ['変化', '成長', '進化', '発展'],
        'integration': ['統合', '融合', '一体', '結合'],
        'wisdom': ['知恵', '洞察', '理解', '気づき']
      },
      practical: {
        'planning': ['計画', '準備', '段取り', '方針'],
        'action': ['行動', '実行', '実践', '取り組み'],
        'reflection': ['振り返り', '反省', '考察', '見直し']
      }
    };
  },
  ...
  createUrgencyFramework() {
    return {
      ...
      timescales: ['immediate', 'today', 'this_week', 'this_month', 'future'],
      consequences: ['minor', 'moderate', 'significant', 'major', 'catastrophic'],
  ...
  createScopeFramework() {
    return {
      impact: ['personal', 'interpersonal', 'team', 'organization', 'society'],
      reach: ['local', 'regional', 'national', 'international', 'global'],
      duration: ['temporary', 'short-term', 'medium-term', 'long-term', 'permanent'],
      depth: ['surface', 'moderate', 'deep', 'fundamental', 'transformational']
    };
  },

  createNatureFramework() {
    return {
      types: ['problem', 'opportunity', 'crisis', 'routine', 'creative'],
  ...
  createAdaptationFramework() {
    return {
      strategies: ['accommodation', 'assimilation', 'transformation', 'resistance'],
      flexibility: ['rigid', 'adaptive', 'flexible', 'fluid'],
      resilience: ['fragile', 'robust', 'antifragile'],
      learning: ['surface', 'deep', 'transformational']
    };
  }
};
