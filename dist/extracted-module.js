...
        /**
        ...
        async function initializeFutureSimulator() {
          ...
          try {
            ...
            // H64_DATAの確認（グローバルスコープから）
            if ((typeof H64_DATA !== 'undefined' && Array.isArray(H64_DATA)) || 
                (typeof window.H64_DATA !== 'undefined' && Array.isArray(window.H64_DATA))) {
            ...
            // Phase 3.5: 🔯 易経変化法則統合システム初期化 - 易経専門エージェント実装
            console.log('🔯 Phase 3.5: 易経変化法則統合システム初期化...');
            
            // 易経変化エンジンの初期化
            let transformationEngine = null;
            let futureBranchingSystem = null;
            let ichingValidator = null;
            
            try {
              // IChingOrthodoxyValidatorの初期化
              if (typeof IChingOrthodoxyValidator !== 'undefined') {
                ichingValidator = new IChingOrthodoxyValidator();
                console.log('✅ 易経正統性検証システム初期化完了');
              } else {
                console.warn('⚠️ IChingOrthodoxyValidator利用不可 - 基本検証で継続');
              }
              
              // IChingTransformationEngineの初期化
              if (typeof IChingTransformationEngine !== 'undefined') {
                transformationEngine = new IChingTransformationEngine();
                console.log('✅ 易経変化エンジン初期化完了 - 5つの変化原理統合');
                
                // 序卦伝論理の検証
                const sequenceTest = transformationEngine.sequenceLogic.creation_sequence[1];
                if (sequenceTest && sequenceTest.name === "乾") {
                  console.log('✅ 易経論理システム検証成功: 正しい流れを確認');
                }
              } else {
                console.error('❌ IChingTransformationEngine利用不可 - 従来システムで継続');
              }
              
              // FutureBranchingSystemの初期化
              if (typeof FutureBranchingSystem !== 'undefined' && transformationEngine) {
                futureBranchingSystem = new FutureBranchingSystem(transformationEngine);
                console.log('✅ 未来分岐システム初期化完了 - 包括的易経変化実装');
                
                // システム品質の検証
                // 最高品質システムを使用
                const complexityLevels = futureBranchingSystem.complexityLevels;
                const level5 = complexityLevels[5];
                if (level5) {
                  console.log(`✨ 最高品質システム: ${level5.name} - 高品質な分析を提供`);
                }
                
                // 未来分岐システムをグローバルスコープに設定
                window.futureBranchingSystem = futureBranchingSystem;
                window.transformationEngine = transformationEngine;
                window.ichingValidator = ichingValidator;
                
              } else {
                console.error('❌ FutureBranchingSystem初期化失敗 - 依存システム不足');
              }
              
              console.log('✅ Phase 3.5完了: 易経変化法則統合システム初期化完了');
              
              // システム品質情報を出力
              console.log('📊 システム品質:');
              console.log('   機能: 総合的な未来予測分析');
              console.log('   設計: ユーザーフレンドリーな体験');
              console.log('   品質: 高品質で信頼性の高いシステム');
              
            } catch (error) {
              console.error('❌ Phase 3.5エラー:', error);
              console.log('🔄 従来システムで継続実行します');
            }
...
      }
    ...
    const originalShowResults = window.showResults || window.updateUI;
    window.showResults = function(data) {
      ...
      setTimeout(() => {
        if (data && data.paths) {
          // 新しいデータ形式に対応
          updateUI(data.paths, data.reasoning, data.explanation, data.analysisEvidence);
        } else if (originalShowResults) {
          // 従来の形式にも対応
        ...
        // summaryChartの表示は個別にshowResults内で制御される
      }, 1000);
    };
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const handleAnalysis = async (isManual = false) => {
            ...
            // 複雑度レベル取得
            // 常に最高品質分析を使用
            const complexityLevel = 5;
            
            console.log(`🎯 選択された複雑度レベル: ${complexityLevel}`);
            ...
            try {
                if (isManual) {
                    ...
                    // コンテキスト分析モーダルを削除 - 自動的に進める
                    const confirmResult = true; // 常に「はい」を選択したものとして処理
                    console.log('🚀 コンテキスト分析を自動的に進めます');

                    // ユーザーの選択に応じて処理を分岐（削除済み）
                    ...
                    if (mlEnhancedResult && mlEnhancedResult.ml_enhanced) {
                        ...
                        // フォールバック結果でも正しい値を取得する（8つのパス生成を確実にするため）
                        hexagramNumber = result.卦番号 || result.hexagram;
                        lineNumber = result.爻番号 || result.line;
                        reasoning = result.根拠 || result.reasoning;
                        explanation = result.解説;
                        
                        // デバッグログ：フォールバック処理確認
                        console.log('🔍 分析結果デバッグ:', {
                            hasResult: !!result,
                            method: result.method || result.analysisResult?.method,
                            hexagram: hexagramNumber,
                            line: lineNumber,
                            isFromFallback: result.method === 'fallback_error_handler'
                        });
                    }
                }

                // 8つのパス生成（選択された複雑度レベルで実行）
                ...
                console.log('🎯 generateAllPaths結果:', {
                    inputHexagram: hexagramNumber,
                    inputLine: lineNumber,
                    complexityLevel: complexityLevel,
                    pathsGenerated: resultPaths ? resultPaths.length : 0,
                    expectedPaths: 8,
                ...
                if (resultPaths && resultPaths.length > 0) {
                    ...
                    // 8つのパスが生成されていない場合は警告ログ
                    if (resultPaths.length !== 8) {
                        console.warn('⚠️ 期待値8個に対し', resultPaths.length, '個のパスが生成されました');
                    }
                    
                    // 🔯 新しい易経変化システムの適用
                    if (window.futureBranchingSystem && complexityLevel > 1) {
                        console.log(`🌟 包括的変化システム適用開始 - レベル${complexityLevel}`);
                        try {
                            const transformationResult = window.futureBranchingSystem.calculateFutureBranches({
                                currentHexagram: hexagramNumber,
                                changingLines: [lineNumber],
                                complexityLevel: complexityLevel,
                            ...
                            console.log(`✅ 包括的変化分析完了: ${transformationResult.totalBranches}分岐生成 (精度${transformationResult.accuracy}%)`);
                            
                            // 結果をevidenceに追加
                            if (analysisEvidence) {
                                analysisEvidence.transformationSystem = {
                                    level: complexityLevel,
                                    accuracy: transformationResult.accuracy,
                                    authenticity: transformationResult.authenticity,
                                    totalBranches: transformationResult.totalBranches,
                                    computationTime: transformationResult.computationTime,
                                    recommendedBranches: transformationResult.recommendedBranches
                                };
                            }
                        } catch (transformError) {
                            console.warn('⚠️ 包括的変化システム適用でエラー:', transformError);
                        }
                    }
                    
                    // 📊 パス品質情報をanalysisEvidenceに追加
                    if (analysisEvidence && resultPaths.length > 0) {
                        const pathQualityInfo = extractPathQualityInfo(resultPaths, complexityLevel);
                        if (!analysisEvidence.transformationSystem) {
                            analysisEvidence.transformationSystem = {};
                        }
                        Object.assign(analysisEvidence.transformationSystem, pathQualityInfo);
                        
                        console.log('📊 パス品質情報を統合:', pathQualityInfo);
                    }
                    
                    // showResults関数を使用してプログレッシブローディングを活用
                    window.showResults({
                        paths: resultPaths,
                        reasoning: reasoning,
                    ...
                    console.error('❌ generateAllPathsが空の結果を返しました:', {
                        hexagramNumber,
                        lineNumber,
                        startDataExists: !!findLineDataByNum(hexagramNumber, lineNumber)
                    });
        ...
        function setupExportEventListeners() {
            ...
            if (exportPDFBtn) {
                exportPDFBtn.addEventListener('click', () => handleExport('pdf-ready'));
            }
            
            // エクスポート履歴表示
            const showExportHistoryBtn = document.getElementById('showExportHistoryBtn');
            if (showExportHistoryBtn) {
                showExportHistoryBtn.addEventListener('click', showExportHistory);
            }
        }
        ...
        async function handleExport(format) {
            ...
            try {
                console.log(`📦 ${format}形式エクスポート開始`);
                
                // プログレス表示開始
                showExportProgress();
                updateExportProgress(10, 'データ取得中...');
                ...
                switch (format) {
                    case 'json':
                        updateExportProgress(30, 'JSON形式でエクスポート中...');
                        result = await dataExportAPI.exportCompleteData({
                            format: 'json',
                    ...
                    case 'csv':
                        updateExportProgress(30, 'CSV形式でエクスポート中...');
                        result = await dataExportAPI.exportToCSV({
                            includeHeaders: true,
                            anonymize: true
                        });
                        break;
                    ...
                    case 'pdf-ready':
                        updateExportProgress(30, 'PDF用データ準備中...');
                        result = await dataExportAPI.preparePDFReportData({
                            includeCharts: true,
                ...
                if (result.success) {
                    updateExportProgress(70, 'ファイル生成中...');
                    
                    // ファイルダウンロード処理
                    await downloadExportFile(result, format);
                    
                    updateExportProgress(100, 'エクスポート完了！');
                    
                    // 成功メッセージ表示
                    setTimeout(() => {
                        hideExportProgress();
                        showExportSuccessMessage(result, format);
                    }, 1000);
                    
                    console.log(`✅ ${format}形式エクスポート完了`);
                } else {
                    throw new Error(result.error || 'エクスポートに失敗しました');
                }
                
            } catch (error) {
                console.error(`❌ ${format}形式エクスポートエラー:`, error);
                hideExportProgress();
        ...
        function updateExportProgress(percentage, status) {
            ...
            const percentageSpan = document.getElementById('exportPercentage');
        ...
        function disableExportButtons() {
            const buttons = ['exportJSONBtn', 'exportCSVBtn', 'exportPDFBtn'];
        ...
        async function downloadExportFile(result, format) {
            try {
                let filename, content, mimeType;
                
                switch (format) {
                    case 'json':
                        filename = `haqei-analysis-${new Date().toISOString().split('T')[0]}.json`;
                        content = JSON.stringify(result.data, null, 2);
                        mimeType = 'application/json';
                        break;
                        
                    case 'csv':
                        filename = `haqei-analysis-${new Date().toISOString().split('T')[0]}.csv`;
                        content = result.data;
                        mimeType = 'text/csv;charset=utf-8';
                        break;
                        
                    case 'pdf-ready':
                        filename = `haqei-report-data-${new Date().toISOString().split('T')[0]}.json`;
                        content = JSON.stringify(result.data, null, 2);
                        mimeType = 'application/json';
                        break;
                }
        ...
        function showExportSuccessMessage(result, format) {
            const formatNames = {
                'json': 'JSON',
                'csv': 'CSV',
                'pdf-ready': 'PDF準備データ'
            ...
            const message = `${formatNames[format]}形式でのエクスポートが完了しました。\n\n` +
                          `- レコード数: ${result.metadata?.recordsCount || 0}件\n` +
                          `- エクスポート時間: ${(result.metadata?.exportTime || 0).toFixed(2)}ms\n` +
                          `- データサイズ: ${formatFileSize(result.metadata?.dataSize || 0)}`;
                          
            alert(message);
        }
        
        function showExportErrorMessage(errorMessage) {
            alert(`エクスポートエラー:\n${errorMessage}`);
        }
        ...
        async function showExportHistory() {
            ...
            try {
                const historyResult = await dataExportAPI.getExportHistory(20);
                
                if (historyResult.success) {
                    displayExportHistoryModal(historyResult.history);
                } else {
                    alert('エクスポート履歴の取得に失敗しました。');
                }
            } catch (error) {
                console.error('❌ エクスポート履歴取得エラー:', error);
                alert('エクスポート履歴の取得中にエラーが発生しました。');
            }
        }
        
        function displayExportHistoryModal(history) {
            const historyHTML = history.length > 0 ? 
                history.map(record => `
                    <div class="border-b border-gray-600 pb-3 mb-3">
                        <div class="flex justify-between items-start">
                            <div>
                                <span class="font-medium text-white">${record.type}</span>
                                <span class="ml-2 text-sm ${record.success ? 'text-green-400' : 'text-red-400'}">
                        ...
                        ${record.recordsCount ? `<div class="text-sm text-gray-400">レコード数: ${record.recordsCount}件</div>` : ''}
                        ${record.error ? `<div class="text-sm text-red-400">エラー: ${record.error}</div>` : ''}
                    </div>
                `).join('') :
                '<p class="text-gray-400 text-center">エクスポート履歴はありません。</p>';
            ...
            if (modalTitle && modalContent) {
                modalTitle.textContent = 'エクスポート履歴';
    ...
    function updateUI(sortedPaths, reasoning = null, explanation = null, analysisEvidence = null) {
        ...
        // 🔯 易経変化システムの品質情報表示
        if (analysisEvidence && analysisEvidence.transformationSystem) {
            const transformSection = document.getElementById('transformationQualitySection');
            const transformData = analysisEvidence.transformationSystem;
            
            if (transformSection) {
                transformSection.style.display = 'block';
                
                // 精度表示
                const accuracyEl = document.getElementById('transformationAccuracy');
                if (accuracyEl) {
                    accuracyEl.textContent = `${transformData.accuracy}%`;
                    accuracyEl.className = transformData.accuracy >= 85 ? 'text-lg font-bold text-green-400' :
                                         transformData.accuracy >= 70 ? 'text-lg font-bold text-yellow-400' :
                                         'text-lg font-bold text-orange-400';
                }
                
                // 正統性表示
                const authenticityEl = document.getElementById('transformationAuthenticity');
                if (authenticityEl) {
                    authenticityEl.textContent = `${transformData.authenticity}%`;
                    authenticityEl.className = transformData.authenticity >= 95 ? 'text-lg font-bold text-indigo-400' :
                                             transformData.authenticity >= 80 ? 'text-lg font-bold text-blue-400' :
                                             'text-lg font-bold text-gray-400';
                }
                
                // レベル情報
                const levelEl = document.getElementById('transformationLevel');
                if (levelEl) {
                    const levelNames = {
                        1: '基本分析',
                        2: '関係分析',
                    ...
                    levelEl.textContent = levelNames[transformData.level] || `分析${transformData.level}`;
                }
                
                // 分岐数表示
                const branchesEl = document.getElementById('transformationBranches');
                if (branchesEl) {
                    branchesEl.textContent = transformData.totalBranches;
                }
                
                // 計算時間表示
                const timeEl = document.getElementById('transformationTime');
                if (timeEl) {
                    timeEl.textContent = `計算時間: ${transformData.computationTime}`;
                }
                
                console.log('✅ 易経変化システム品質情報UI更新完了');
            }
        }
        ...
        document.getElementById("currentTitle").innerText = `現在地: ${startState["卦名"] || startState["親となる卦"]} ${startState.爻}`;
    ...
    function renderDetailCards(sortedPaths) {
        ...
        sortedPaths.forEach((path, index) => {
            ...
            const finalStateTitle = `${finalState["卦名"] || finalState["親となる卦"]} ${finalState.爻}`;
            ...
            // 🌟 品質情報の抽出
            const qualityInfo = extractScenarioQuality(path, index);
            const qualityHtml = generateQualityIndicators(qualityInfo);
      ...
      // シナリオ詳細・ヘルプ・現在状態表示用のモーダル関数
      function showModal(content) {
        // シナリオ詳細用のモーダルを動的に作成
        const modalHtml = `
          <div id="modalContainer" class="fixed inset-0 z-50 flex items-center justify-center">
            <div id="modalOverlay" class="absolute inset-0 bg-black/70 modal-overlay"></div>
            <div id="modalContent" class="modal-content bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto p-8 relative transform">
      ...
      function showScenarioModal(path, index, overallTrend, firstStepTrend) {
        ...
        path.forEach((step, stepIndex) => {
          ...
          if (stepIndex < path.length - 1) {
            ...
            const targetName = `${step["卦名"] || step["親となる卦"]} ${step.爻}`;
            const normalizedTargetName = `${step["卦名"] || step["親となる卦"]} ${step.爻}`
              .replace(/\s+/g, " ")
              .trim();
        ...
        <div class="py-4 px-4 border-b border-gray-700 last:border-b-0 ${stepWrapperClass}">
          ...
          }: ${step["卦名"] || step["親となる卦"]} ${step.爻} ${stepTitleIcon}</h4>
  ...
  `;
      ...
      // hideModal関数は削除済み（コンテキスト分析モーダル廃止）
      ...
      function setupInteraction(sortedPaths) {
        ...
        // 分析サマリーを選択に応じて更新する関数
        const updateAnalysisSummary = (choiceType) => {
          const summaryContent = document.getElementById('summary-content');
          const summaryCard = document.getElementById('summaryCard');
          
          if (!summaryContent || !summaryCard || !currentAnalysisData) return;
          
          // 現在の分析データから適切な情報を抽出
          const currentState = currentAnalysisData.currentState || {};
          const choiceData = generateChoiceSummary(choiceType, currentState, sortedPaths);
          
          // サマリーカードの境界色を選択に応じて変更
          summaryCard.classList.remove('border-teal-500/50', 'border-purple-500/50');
          if (choiceType === 'stagnate') {
            summaryCard.classList.add('border-teal-500/50');
          } else if (choiceType === 'change') {
            summaryCard.classList.add('border-purple-500/50');
          }
          
          // サマリー内容を更新
          summaryContent.innerHTML = `
            <div class="bg-yellow-400/10 border-l-4 border-yellow-400 p-3 rounded-md">
              <div class="flex items-start">
                <div class="flex-shrink-0">
                  <span class="text-yellow-400 text-lg">⚡</span>
                </div>
                <div class="ml-3">
                  <h3 class="text-sm font-medium text-yellow-200">${choiceData.title}</h3>
                  <div class="mt-2 text-sm text-yellow-100">
                    <p class="mb-2">${choiceData.description}</p>
                    <div class="space-y-1">
                      <div><strong>重点シナリオ:</strong> ${choiceData.focusScenarios}</div>
                      <div><strong>推奨アプローチ:</strong> ${choiceData.approach}</div>
                      <div><strong>期待される結果:</strong> ${choiceData.expectedOutcome}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        // 選択タイプに応じたサマリー情報を生成
        const generateChoiceSummary = (choiceType, currentState, paths) => {
          const relatedPaths = paths.filter(path => path[1]?.choice === choiceType);
          const avgScore = relatedPaths.reduce((sum, path) => sum + (path[path.length - 1]?.S7_総合評価スコア || 0), 0) / relatedPaths.length;
          
          if (choiceType === 'stagnate') {
            return {
              title: '「進」の道 - 現状維持・安定性重視の戦略',
              description: '現在の状況を活かしながら、着実に改善を図る安定的なアプローチです。',
              focusScenarios: `上位${Math.ceil(relatedPaths.length/2)}シナリオ（安定性重視）`,
              approach: '既存の強みを活かし、リスクを最小化しながら段階的改善',
              expectedOutcome: `平均スコア ${avgScore.toFixed(1)}点 - 安定した成長軌道`
            };
          } else {
            return {
              title: '「変」の道 - 変化・成長志向の戦略',
              description: '積極的な変化を通じて、新たな可能性を切り開く挑戦的なアプローチです。',
              focusScenarios: `上位${Math.ceil(relatedPaths.length/2)}シナリオ（変化重視）`,
              approach: '新たな挑戦を通じて、潜在能力を最大限に発揮',
              expectedOutcome: `平均スコア ${avgScore.toFixed(1)}点 - 飛躍的成長の可能性`
            };
          }
        };

        // サマリーを元の状態に戻す関数
        const resetAnalysisSummary = () => {
          const summaryContent = document.getElementById('summary-content');
          const summaryCard = document.getElementById('summaryCard');
          
          if (!summaryContent || !summaryCard) return;
          
          // 境界色をリセット
          summaryCard.classList.remove('border-teal-500/50', 'border-purple-500/50');
          
          // 元のサマリー内容に戻す（元の分析結果を表示）
          if (currentAnalysisData && currentAnalysisData.currentState) {
            const state = currentAnalysisData.currentState;
            summaryContent.innerHTML = `
              <div class="bg-yellow-400/10 border-l-4 border-yellow-400 p-3 rounded-md">
                <div class="flex items-start">
                  <div class="flex-shrink-0">
                  ...
                  <div class="ml-3">
                    <h3 class="text-sm font-medium text-yellow-200">現在の状況分析</h3>
                    <div class="mt-2 text-sm text-yellow-100">
                      <p>あなたの現在地は <strong>${state.hexagramName || '状況卦'}</strong> です。</p>
                      <div class="mt-2 space-y-1">
                        <div><strong>総合評価:</strong> ${state.S7_総合評価スコア || 'N/A'}点</div>
                        <div><strong>状況:</strong> ${state.現代解釈の要約 || '分析中...'}</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        if (choiceCardShin) {
          choiceCardShin.addEventListener("mouseover", () => {
            ...
            updateAnalysisSummary("stagnate"); // 分析サマリー更新
          });
          choiceCardShin.addEventListener("mouseleave", () => {
            resetAllHighlights();
            resetAnalysisSummary(); // サマリーも元に戻す
          });
        }
        ...
        if (choiceCardHen) {
          choiceCardHen.addEventListener("mouseover", () => {
            ...
            updateAnalysisSummary("change"); // 分析サマリー更新
          });
          choiceCardHen.addEventListener("mouseleave", () => {
            resetAllHighlights();
            resetAnalysisSummary(); // サマリーも元に戻す
          });
        }
       ...
       * データが見つからない場合は代替処理を実行
       */
      const findHexDataWithFallback = (hexNum) => {
        // グローバルスコープからH64_DATAを取得
        const hexData = window.H64_DATA || H64_DATA;
        
        console.log(`🔍 findHexDataWithFallback呼び出し:`, { 
          hexNum, 
          hexNumType: typeof hexNum,
          H64_DATA存在: !!hexData,
          H64_DATA長さ: hexData ? hexData.length : 'null',
          windowH64_DATA: !!window.H64_DATA
        });
        ...
        if (result) {
          console.log(`✅ findHexDataWithFallback結果:`, { 
            hexNum, 
            result: `見つかった: ${result.名前}` 
          });
          return result;
        } else {
      ...
      const findHexData = findHexDataWithFallback;
      ...
      const logDebugInfo = (context, data) => {
        ...
        if (typeof data === 'object' && data !== null) {
          Object.keys(data).forEach(key => {
            ...
            if (value === undefined) {
              console.warn(`⚠️  ${key}: UNDEFINED (これがエラーの原因の可能性)`);
            } else if (value === null) {
              console.warn(`⚠️  ${key}: NULL`);
            } else {
      ...
      const createFallbackLineData = (hexNum, lineNum, step, choice) => {
        ...
        console.log(`🔧 createFallbackLineData: 卦${hexNum}(${hexName})爻${lineNum}(${lineName})のフォールバック爻データを作成`);
      ...
      const createFallbackHexData = (hexNum) => {
        ...
        console.log(`🔧 createFallbackHexData: 卦${hexNum}(${hexName})のフォールバックデータを作成`);
        ...
        return {
          ...
          五爻変: lineChanges.五爻変,
          上爻変: lineChanges.上爻変,
          __fallback: true  // フォールバックデータであることを示すフラグ
        };
      };

      /**
       * 爻変の手動計算関数
       * 易経の基本原理に基づいて爻変後の卦を計算
       */
      const calculateLineChangeManually = (hexNum) => {
        console.log(`🧮 calculateLineChangeManually: 卦${hexNum}の爻変を手動計算中`);
        ...
        if (!currentBinary) {
          console.error(`❌ 卦${hexNum}の二進数表現が見つかりません`);
        ...
        const calculateChange = (binary, position) => {
          ...
          return binaryToHex[newBinary] || hexNum;  // 見つからない場合は元の卦番号を返す
        };

        const result = {
          初爻変: calculateChange(currentBinary, 0),  // 最下位（初爻）
          ...
          上爻変: calculateChange(currentBinary, 5)   // 最上位（上爻）
        };

        console.log(`✅ calculateLineChangeManually結果:`, { hexNum, ...result });
        return result;
      };
      ...
      const getNextState = (currentState, choice) => {
        ...
        console.log(`🔍 getNextState呼び出し:`, { 
          choice, 
          currentHexNum, 
          currentLineNum, 
          currentState爻: currentState.爻 
        });
        ...
        if (choice === "stagnate") {
          ...
          console.log(`✅ stagnate処理:`, { nextHexNum, nextLineNum });
        } else {
          console.log(`🔄 change処理開始:`, { currentHexNum, currentLineNum });
          
          const currentHexData = findHexData(currentHexNum);
          console.log(`📊 currentHexData取得:`, { 
            currentHexData: currentHexData ? '取得成功' : 'null',
            hexName: currentHexData?.名前
          });
          
          if (!currentHexData) {
            console.error(`❌ currentHexDataが見つかりません: hexNum=${currentHexNum}`);
            return null;
          }
          ...
          const keyIndex = currentLineNum - 1;
          const lineKey = lineKeys[keyIndex];
          nextHexNum = currentHexData[lineKey];
          nextLineNum = currentLineNum;
          
          console.log(`🎯 change詳細:`, { 
            keyIndex, 
            lineKey, 
            nextHexNum, 
            nextLineNum,
            availableKeys: Object.keys(currentHexData)
          });
        }
      ...
      /**
       * 包括的8シナリオ生成システム - 5レベル複雑度対応
       * 
       * 🌟 世界最高水準の易経変化システム統合実装
       * - 基本分析: シンプルな変化予測システム
       * - 段階的高品質化: より詳細な分析オプション
       * - 総合分析: 包括的な変化予測システム
       * 
       * @param {number} startHex - 開始卦番号
       * @param {number} startLine - 開始爻番号
       * @param {number} complexityLevel - 複雑度レベル (1-5)
       * @param {Object} options - 追加オプション
       * @returns {Array} 8つのシナリオパス
       */
      function generateAllPaths(startHex, startLine, complexityLevel = 5, options = {}) {
        ...
        console.log('📍 startData取得結果:', { 
          startData: startData ? '取得成功' : 'null', 
          hexagonName: startData?.卦名,
          line: startData?.爻 
        });
        
        if (!startData) {
          console.error('❌ startDataが見つからないため、パス生成を中断');
          return [];
        }
      ...
      // 簡易分析機能を削除し、常に最高品質の総合分析を使用
      // これによりコードサイズを大幅削減し、パフォーマンスを向上
      ...
      function generateComprehensivePaths(startData, startHex, startLine, options = {}) {
        console.log('✨ 未来予測分析開始 - 総合システム実行中');
        
        // IChingTransformationEngine & FutureBranchingSystemとの統合
        if (!window.futureBranchingSystem || !window.ichingTransformationEngine) {
          console.warn('⚠️ 総合システム未初期化、基本分析に切り替え');
          return generateBasicPaths(startData, startHex, startLine);
        }
        
        // システム準備状況の確認
        if (typeof window.futureBranchingSystem.calculateFutureBranches !== 'function') {
          console.warn('⚠️ 未来分岐システム未完全初期化、基本分析に切り替え');
          return generateBasicPaths(startData, startHex, startLine);
        }
        ...
        try {
          ...
          const transformationResult = window.futureBranchingSystem.calculateFutureBranches({
            ...
            personalContext: {
              ...
              osAnalysisData: options.osAnalysisData || null
            },
            preferredStrategy: "quantum"
          });
       ...
       */
      /**
       * 強化されたgetHexagramData関数（フォールバック機能付き）
       */
      function getHexagramData(hexNum) {
        const hexData = findHexData(hexNum);
        if (!hexData) {
          console.warn(`❌ getHexagramData: 卦番号 ${hexNum} のデータが見つかりません - フォールバック処理を実行`);
          return createFallbackHexagramData(hexNum);
        }
        
        // フォールバックデータの場合は特別処理
        if (hexData.__fallback) {
          console.log(`🔧 getHexagramData: フォールバックデータ使用 - 卦${hexNum}`);
          return {
            卦番号: hexData.卦番号,
            親となる卦: hexData.名前,
            卦名: hexData.名前,
            ...
            S5_主体性: "中立",
            S6_変動性スコア: 50,
            現代解釈の要約: `${hexData.名前}の基本的な状況。フォールバックデータのため詳細情報は限定的です。`,
            __fallback: true
          };
        }
        ...
        return {
          卦番号: hexData.卦番号,
          親となる卦: hexData.名前 || hexData.親となる卦,
          卦名: hexData.名前 || hexData.親となる卦,
          // スコアのデフォルト値
          S1_基本スコア: hexData.基本スコア || 50,
          S2_ポテンシャル: hexData.ポテンシャル || 50,
          S3_安定性スコア: hexData.安定性 || 50,
          S4_リスク: hexData.リスク || -35,
          S5_主体性: hexData.主体性 || "受動",
          S6_変動性スコア: hexData.変動性 || 50
        };
      }
       ...
       * フォールバック用卦グラムデータ作成関数
       */
      function createFallbackHexagramData(hexNum) {
        // 基本的な卦名のマッピング
        const hexNames = {
          1: '乾為天', 2: '坤為地', 3: '水雷屯', 4: '山水蒙', 5: '水天需', 6: '天水訟', 7: '地水師', 8: '水地比',
          9: '風天小畜', 10: '天澤履', 11: '地天泰', 12: '天地否', 13: '天火同人', 14: '火天大有', 15: '地山謙', 16: '雷地豫',
        ...
        console.log(`🔧 createFallbackHexagramData: 卦${hexNum}(${hexName})のフォールバック卦データを作成`);
        ...
        return {
          ...
          S5_主体性: "中立",
          S6_変動性スコア: 50,
          現代解釈の要約: `${hexName}の基本的な状況を示しています。フォールバックデータのため、詳細な解釈は限定的です。`,
          __fallback: true
        };
      }

      /**
       * 基本的な8つのパスを生成（フォールバック用）
       */
      function generateBasicPaths(startData, startHex, startLine) {
        console.log('🔄 基本パス生成を実行');
        
        // 入力パラメータのデバッグ情報
        logDebugInfo('generateBasicPaths入力パラメータ', {
          startData,
          startHex,
          startLine,
          startDataKeys: startData ? Object.keys(startData) : 'undefined'
        });
        ...
        for (let i = 0; i < 8; i++) {
          ...
          if (isChangePath) {
            // 変パス
            const hexData = findHexData(startHex);
            let changeHex = startHex;
            
            if (hexData) {
              // 爻番号に応じた変化後の卦を取得
              const lineNames = ["初爻変", "二爻変", "三爻変", "四爻変", "五爻変", "上爻変"];
              const lineName = lineNames[startLine - 1];
              changeHex = hexData[lineName] || startHex;
            }
            
            const changeHexData = getHexagramData(changeHex);
            if (changeHexData && changeHexData.卦番号) {
              path.push({
                ...changeHexData,
                step: 1,
                choice: "change",
                爻: `${["初", "二", "三", "四", "五", "上"][startLine - 1]}`,
                爻数値: startLine
              });
            } else {
              // エラー: 変化データが取得できない
              console.error(`❌ 変化データ取得エラー: 卦${changeHex}のデータが見つかりません`);
              path.push({
                卦番号: changeHex,
                卦名: `ERROR_卦${changeHex}`,
                爻: `ERROR_${["初", "二", "三", "四", "五", "上"][startLine - 1]}`,
                爻数値: startLine,
                状況説明: "データ取得エラー: 変化後の卦データが見つかりません",
                step: 1,
                choice: "change",
                S1_基本スコア: 0,
                S2_ポテンシャル: 0,
            ...
            // 進パス
            const nextLine = ((startLine + i) % 6) + 1;
            const lineData = findLineDataByNum(startHex, nextLine);
            ...
            if (lineData && Object.keys(lineData).length > 0) {
              ...
              // フォールバック処理: lineDataが見つからない場合
              console.warn(`⚠️  進パス: 卦${startHex}爻${nextLine}のデータが見つかりません - フォールバック処理実行`);
              path.push(createFallbackLineData(startHex, nextLine, 1, "stagnate"));
            }
          }
          
          // ステップ2-3: 追加の変化
          for (let step = 2; step <= 3; step++) {
            const prevStep = path[step - 1];
            const nextLine = ((prevStep.爻数値 + step - 1) % 6) + 1;
            
            if (isChangePath && Math.random() > 0.5) {
              // 変化を継続
              const currentHex = prevStep.卦番号 || startHex;
              const hexData = findHexData(currentHex);
              let changeHex = currentHex;
              
              if (hexData) {
                // 爻番号に応じた変化後の卦を取得
                const lineNames = ["初爻変", "二爻変", "三爻変", "四爻変", "五爻変", "上爻変"];
                const lineName = lineNames[nextLine - 1];
                changeHex = hexData[lineName] || currentHex;
              }
              ...
              if (changeHexData && changeHexData.卦番号) {
                path.push({
                  ...
                  step: step,
                  choice: "change",
                  爻: `${["初", "二", "三", "四", "五", "上"][nextLine - 1]}`,
                  爻数値: nextLine
                });
              } else {
                // エラー: 変化データが取得できない
                console.error(`❌ ステップ${step}変化データ取得エラー: 卦${changeHex}のデータが見つかりません`);
                path.push({
                  卦番号: changeHex,
                  卦名: `ERROR_卦${changeHex}`,
                  爻: `ERROR_${["初", "二", "三", "四", "五", "上"][nextLine - 1]}`,
                  爻数値: nextLine,
                  状況説明: `ステップ${step}データ取得エラー: 変化後の卦データが見つかりません`,
                  step: step,
                  choice: "change",
                  S1_基本スコア: 0,
                  S2_ポテンシャル: 0,
              ...
              // 進行を継続
              const lineData = findLineDataByNum(prevStep.卦番号 || startHex, nextLine);
              ...
              if (lineData && Object.keys(lineData).length > 0) {
                ...
                // フォールバック処理: lineDataが見つからない場合
                console.warn(`⚠️  ステップ${step}進行: 卦${prevStep.卦番号 || startHex}爻${nextLine}のデータが見つかりません - フォールバック処理実行`);
                path.push(createFallbackLineData(prevStep.卦番号 || startHex, nextLine, step, "stagnate"));
              }
            }
          }
          
          // 各パスの完整性をチェック
          const pathValid = path.every(step => {
            const hasRequiredFields = step && step.卦名 && step.爻 !== undefined;
            if (!hasRequiredFields) {
              console.warn(`⚠️  パス${i}のステップに不完全なデータ:`, step);
            }
          ...
          if (pathValid) {
            allPaths.push(path);
            console.log(`✅ パス${i}生成完了 (${isChangePath ? '変' : '進'}パス)`);
          } else {
            console.error(`❌ パス${i}生成失敗 - データ不完全`);
            // エラーが発生した場合でも、フォールバックパスを追加
            const fallbackPath = createFallbackPath(startData, startHex, startLine, i);
            allPaths.push(fallbackPath);
            console.log(`🔧 パス${i}フォールバック生成完了`);
          }
        }
        
        console.log(`🎯 generateBasicPaths完了 - 生成パス数: ${allPaths.length}`);
        logDebugInfo('generateBasicPaths結果サマリー', {
          pathsCount: allPaths.length,
          firstPathSteps: allPaths[0] ? allPaths[0].length : 0,
          hasUndefinedInFirstPath: allPaths[0] ? allPaths[0].some(step => 
       ...
       * 完全フォールバックパス作成関数
       */
      const createFallbackPath = (startData, startHex, startLine, pathIndex) => {
        console.log(`🔧 createFallbackPath: パス${pathIndex}の完全フォールバックパスを生成`);
        
        const isChangePath = pathIndex >= 4;
        const path = [];
        
        // ステップ0: 現在地（startDataをベースに安全なデータを作成）
        const safeStartData = {
          卦番号: startHex,
          卦名: startData?.卦名 || `卦${startHex}`,
          爻: startData?.爻 || `${["初", "二", "三", "四", "五", "上"][startLine - 1]}爻`,
          爻数値: startLine,
          現代解釈の要約: startData?.現代解釈の要約 || "フォールバックデータによる基本的な解釈",
          S1_基本スコア: startData?.S1_基本スコア || 50,
          S2_ポテンシャル: startData?.S2_ポテンシャル || 50,
          S3_安定性スコア: startData?.S3_安定性スコア || 50,
          S4_リスク: startData?.S4_リスク || -35,
          S5_主体性推奨スタンス: startData?.S5_主体性推奨スタンス || '中立',
          S6_変動性スコア: startData?.S6_変動性スコア || 50,
          S7_総合評価スコア: startData?.S7_総合評価スコア || 50,
          choice: isChangePath ? "change" : "stagnate",
          __fallback: true
        };
        path.push(safeStartData);
        
        // ステップ1-3: 安全なフォールバックデータで埋める
        for (let step = 1; step <= 3; step++) {
          const nextLine = ((startLine + step - 1) % 6) + 1;
          const stepData = createFallbackLineData(startHex, nextLine, step, isChangePath ? "change" : "stagnate");
          path.push(stepData);
        }
      ...
      function convertTransformationToPaths(transformationResult, startData, startHex, startLine) {
        const { branches, totalBranches, accuracy, authenticity } = transformationResult;
        
        if (!branches || branches.length === 0) {
          console.warn('⚠️ 包括変化結果が空、基本パスを生成');
          return generateBasicPaths(startData, startHex, startLine);
        }
        
        const comprehensivePaths = [];
        
        // 最大8つのブランチを選択（品質順にソート）
        const selectedBranches = branches
          .sort((a, b) => (b.quality?.total || 0) - (a.quality?.total || 0))
          .slice(0, 8);
        ...
        for (let i = 0; i < selectedBranches.length; i++) {
          ...
          // 変化段階を構築（3段階）
          for (let step = 1; step <= 3; step++) {
            const hexNum = branch.sequence ? branch.sequence[step - 1] : calculateNextHexagram(startHex, step, branch);
            const lineNum = calculateNextLine(startLine, step, branch);
            const stepData = findLineDataByNum(hexNum, lineNum);
            
            if (stepData) {
              path.push({
                ...stepData,
                lineNum: lineNum,
                choice: branch.choices ? branch.choices[step - 1] : (step % 2 === 0 ? "change" : "stagnate"),
                変化段階: step,
                分岐種類: branch.type || 'comprehensive',
                確率: branch.probability || 0.8,
                エネルギー: branch.energy || 'balanced',
                包括精度: accuracy,
                正統性: authenticity,
                bunenjin統合: true
              });
          ...
          // 8つに達したら終了
          if (comprehensivePaths.length >= 8) break;
        }
        
        // 不足分は基本パスで補完
        while (comprehensivePaths.length < 8) {
          const basicPaths = generateBasicPaths(startData, startHex, startLine);
          for (const basicPath of basicPaths) {
            if (comprehensivePaths.length >= 8) break;
            
            // 基本パスに包括情報を追加
            const enhancedPath = basicPath.map((step, index) => ({
              ...step,
              包括精度: accuracy,
              正統性: authenticity,
              補完フラグ: true
            ...
            comprehensivePaths.push(enhancedPath);
          }
        }
        
        return comprehensivePaths.slice(0, 8);
      }
       ...
       * 卦の五行属性を取得
       */
      function getHexagramElement(hexNum) {
        const elementMap = {
          1: 'metal', 2: 'earth', 3: 'wood', 4: 'wood', 5: 'water', 6: 'water',
          7: 'earth', 8: 'earth', 9: 'wood', 10: 'metal', 11: 'earth', 12: 'earth',
          13: 'fire', 14: 'fire', 15: 'earth', 16: 'earth', 17: 'wood', 18: 'wood',
          19: 'earth', 20: 'wood', 21: 'fire', 22: 'fire', 23: 'earth', 24: 'earth',
          25: 'metal', 26: 'metal', 27: 'wood', 28: 'wood', 29: 'water', 30: 'fire'
        };
        return elementMap[hexNum] || 'neutral';
      }

      /**
       * 五行エネルギー変化を計算
       */
      function calculateElementalTransition(step, element) {
        const transitions = {
          wood: ['growth', 'expansion', 'creativity'],
          fire: ['passion', 'transformation', 'illumination'],
          earth: ['stability', 'grounding', 'nurturing'],
          metal: ['precision', 'refinement', 'structure'],
          water: ['flow', 'adaptation', 'wisdom']
        };
        return transitions[element] ? transitions[element][step % 3] : 'neutral';
      }

      /**
       * 卦関係性を生成（互・綜・錯）
       */
      function generateHexagramRelations(hexNum) {
        // 簡略化された関係性マップ
        const relations = [
          { type: '互卦', hexNum: (hexNum + 31) % 64 + 1, probability: 0.7 },
          { type: '綜卦', hexNum: 65 - hexNum, probability: 0.6 },
          { type: '錯卦', hexNum: (hexNum + 32) % 64 + 1, probability: 0.5 }
        ];
        return relations;
      }
       ...
       * 序卦伝論理を取得
       */
      function getSequenceLogic(currentHex, nextHex) {
        // 序卦伝の基本論理パターン
        const sequenceMap = {
          1: { necessity: '有天地然後万物生', probability: 0.9 },
          2: { necessity: '有万物然後有男女', probability: 0.8 },
          3: { necessity: '有男女然後有夫婦', probability: 0.85 }
        };
        
        return sequenceMap[currentHex] || { 
          necessity: '変化の必然性', 
          probability: 0.7 
        };
      }
       ...
       * 変化段階を取得
       */
      function getTransformationStage(hexNum) {
        if (hexNum <= 12) return '創造段階';
        if (hexNum <= 24) return '発展段階';
        if (hexNum <= 36) return '修養段階';
        if (hexNum <= 48) return '関係段階';
        return '変革段階';
      }
       ...
       * 次の卦を計算（包括システム用）
       */
      function calculateNextHexagram(startHex, step, branch) {
        if (branch.hexagramTransitions) {
          return branch.hexagramTransitions[step - 1] || startHex;
        }
        
        // フォールバック計算
        return ((startHex + step * 7) % 64) + 1;
      }

      /**
       * 次の爻を計算（包括システム用）
       */
      function calculateNextLine(startLine, step, branch) {
        if (branch.lineTransitions) {
          return branch.lineTransitions[step - 1] || startLine;
        }
        
        // フォールバック計算
        return ((startLine + step) % 6) + 1;
      }
      ...
      function extractPathQualityInfo(paths, complexityLevel) {
        ...
        // パス内に包括精度情報がある場合はそれを使用
        const firstPath = paths[0];
        if (firstPath && firstPath.length > 0) {
          const firstStep = firstPath[0];
          if (firstStep.包括精度) {
            quality.accuracy = firstStep.包括精度;
          }
          if (firstStep.正統性) {
            quality.authenticity = firstStep.正統性;
          }
        }
        ...
        return {
          ...
          computationTime: performance.now() % 1000 // 簡略化された計算時間
        };
      }
       ...
       * シナリオ品質情報を抽出
       */
      function extractScenarioQuality(path, index) {
        if (!path || path.length === 0) {
          return { hasEnhancements: false, level: 1, indicators: [] };
        }
        
        const firstStep = path[0];
        const hasEnhancements = path.some(step => 
          step.五行属性 || step.序卦論理 || step.包括精度 || step.正統性 || step.分岐種類
        );
        
        const level = firstStep.包括精度 ? 
          (firstStep.包括精度 >= 90 ? 5 : firstStep.包括精度 >= 80 ? 4 : firstStep.包括精度 >= 65 ? 3 : 2) : 1;
        
        const indicators = [];
        
        // 五行属性チェック
        if (path.some(step => step.五行属性)) {
          indicators.push({ type: 'element', label: '五', color: 'text-blue-400' });
        }
        
        // 序卦論理チェック
        if (path.some(step => step.序卦論理)) {
          indicators.push({ type: 'sequence', label: '序', color: 'text-purple-400' });
        }
        
        // bunenjin統合チェック
        if (path.some(step => step.bunenjin統合)) {
          indicators.push({ type: 'bunenjin', label: '哲', color: 'text-indigo-400' });
        }
        
        // 包括システムチェック
        if (firstStep.包括精度) {
          indicators.push({ type: 'comprehensive', label: '包', color: 'text-green-400' });
        }
        ...
        return {
          ...
          level,
          indicators,
          accuracy: firstStep.包括精度 || (level === 1 ? 30 : level === 2 ? 50 : level === 3 ? 70 : level === 4 ? 85 : 92),
          authenticity: firstStep.正統性 || (level === 1 ? 40 : level === 2 ? 70 : level === 3 ? 80 : level === 4 ? 95 : 98)
        };
      }
      ...
      function generateQualityIndicators(qualityInfo) {
        if (!qualityInfo.hasEnhancements && qualityInfo.level === 1) {
          return ''; // 基本分析は表示なし
        }
        
        const levelNames = {
          1: '基本', 2: '関係', 3: '五行', 4: '流れ', 5: '総合'
        };
        
        const levelColors = {
          1: 'text-gray-400', 2: 'text-green-400', 3: 'text-blue-400', 
          4: 'text-purple-400', 5: 'text-indigo-400'
        };
        
        const indicatorsHtml = qualityInfo.indicators.length > 0 ? 
          qualityInfo.indicators.map(ind => 
          ...
          <div class="mb-2 pb-2 border-b border-gray-700/30">
            ...
            ${qualityInfo.level >= 3 ? `
              <div class="flex justify-between text-xs mt-1">
                <span class="text-gray-500">精度: <span class="text-green-400">${qualityInfo.accuracy}%</span></span>
                <span class="text-gray-500">正統: <span class="text-blue-400">${qualityInfo.authenticity}%</span></span>
              </div>
            ` : ''}
          </div>
        `;
      ...
      function sendAnonymousData(professionalData) {
        ...
        const anonymousPayload = {
          ...
          start_hexagram:
            professionalData.selectedFuture.path[0]?.["卦名"] || professionalData.selectedFuture.path[0]?.["親となる卦"],
          ...
          selected_scenario_path: professionalData.selectedFuture.path
            ...
            .join("→"),
          final_hexagram:
            professionalData.selectedFuture.path[3]?.["卦名"] || professionalData.selectedFuture.path[3]?.["親となる卦"],
      ...
      function evaluateContextFit(emotionalContext, temporalContext, contextType, lineData) {
        ...
        const hexagramName = lineData["卦名"] || lineData["親となる卦"] || '';
        ...
        try {
          ...
          if (window.HexagramMappingEngine) {
            try {
              ...
              console.log('📊 Phase 2.5デバッグ情報:', {
                hasSituationalResult: !!situationalResult,
                readyForHexagramMapping: situationalResult?.readyForHexagramMapping,
                confidence: situationalResult?.confidence,
                situation: situationalResult?.situation
              });
              
              const hexagramMapper = new window.HexagramMappingEngine();
              await hexagramMapper.initialize(); // 初期化を追加
              
              // readyForHexagramMappingがfalseでも実行（デバッグ用）
              if (situationalResult) {
                ...
                console.warn('⚠️ Phase 2.5: situationalResultが存在しません');
              }
            } catch (hexagramError) {
          ...
          console.log('📊 Phase 2.9前提条件チェック:', {
            hasMetaphorEngine: !!window.MetaphorGenerationEngine,
            hasHexagramMapping: !!hexagramMappingResult,
            hexagramMappingKeys: hexagramMappingResult ? Object.keys(hexagramMappingResult) : []
          });
          ...
          if (window.MetaphorGenerationEngine) {
            try {
              ...
              await metaphorGenerator.initialize(); // 初期化を追加
              
              // hexagramMappingResultがnullでも試行してみる（フォールバック付き）
              if (hexagramMappingResult) {
                console.log('📊 Phase 2.9デバッグ情報:', {
                  mappingConfidence: hexagramMappingResult?.mappingConfidence,
                  primaryHexagram: hexagramMappingResult?.primaryHexagram?.name_jp || 'unknown'
                });
                ...
                console.warn('⚠️ Phase 2.9: hexagramMappingResultがnull - フォールバック実行');
                // フォールバック実行を試行
                metaphorGenerationResult = await metaphorGenerator.generateFallbackMetaphor(
                  situationalResult,
                  userPersona
                );
              }
            ...
            console.warn('⚠️ Phase 2.9: MetaphorGenerationEngineが利用できません');
          }
          
          // Phase 1・2・2.5・2.9結果の統合
          // enhancedResultの存在確認
          if (!enhancedResult) {
            console.warn('⚠️ enhancedResult が取得できませんでした - 基本分析にフォールバック');
            return basicResult;
          }
          
          const integratedResult = {
            // 既存互換性（primary, confidence, secondary, analysis）
            primary: enhancedResult.primaryContext || basicResult.primary,
            confidence: Math.min(Math.max(enhancedResult.confidence || 0, basicResult.confidence * 0.8), 0.95),
            secondary: (enhancedResult.secondaryContexts && enhancedResult.secondaryContexts.length > 0) ? 
            ...
            analysis: {
              ...
              // Phase 1拡張情報
              enhancedAnalysis: {
                isMultidimensional: enhancedResult?.isMultidimensional || false,
                isHSPCase: enhancedResult?.isHSPCase || false,
                ...
                qualityLevel: (enhancedResult?.confidence || 0) >= 0.8 ? 'A級拡張' : 
                            (enhancedResult?.confidence || 0) >= 0.6 ? 'B級拡張' : 'C級拡張',
      ...
      class IntegratedAnalysisEngine {
        ...
        generateHSPSpecificFallback() {
          ...
          return {
            ...
            reasoning: `HSP特性に特化した分析により、感情調整に最適な「${lineData["卦名"] || lineData["親となる卦"]} ${lineData.爻}」を選択。${selected.theme}の時期として理解できます。`,
      ...
      async function generateMLEnhancedReasoning(inputText, userPersona) {
        ...
        return {
          ...
          integrated_analysis: true, // 統合分析システム
      ...
      class StatisticalQualityAssurance {
        ...
        generateQualityReport() {
          ...
          return {
            ...
            comprehensive_integration: 'A級', // 総合分析統合度
      ...
      class FutureSimulatorErrorHandler {
        ...
        static generateFallbackResult(contextType, error, errorType) {
          ...
          const reasoningText = `
            <div class="bg-indigo-900/20 border border-indigo-600/30 p-3 rounded-lg mb-3">
              <h4 class="text-indigo-400 font-medium mb-2">🔯 易経智慧</h4>
              <p class="text-sm text-gray-300">現在の状況を<strong>誠意の卦（中孚）</strong>の精神で分析いたします。</p>
            </div>
            📊 コンテキスト: ${contextName}<br>
            現在の状況は「<strong>${fallback.theme}</strong>」の観点から理解できます。<br>
            <small class="text-indigo-400">🎯 正統な易経原理に基づく分析結果をお届けします</small>
          `;
      ...
      async function callAIAssistant(worryText, h384Data, futureThemeMap, contextType = 'personal') {
        ...
        try {
          if (!integratedEngine) {
            ...
            if (initStatus.overallHealth === 'limited') {
              console.warn('⚠️ 制限モードで動作 - 品質が制限される可能性があります');
            } else {
              console.log('✅ IntegratedAnalysisEngine初期化完了 - A級品質準備完了');
            }
          }
        } catch (error) {
        ...
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            // より長いタイムアウトでA級品質を確保
            setTimeout(() => reject(new Error('分析処理がタイムアウトしました - より詳細な分析のため時間がかかりました')), 25000);
          });
          ...
          // A級品質分析結果の詳細ログ
          console.log('🎯 統合分析完了:', {
            品質グレード: analysisResult.qualityAssessment?.grade || 'unknown',
            信頼度: analysisResult.qualityMetrics?.overallConfidence,
            初期化状態: analysisResult.qualityMetrics?.initializationHealth,
            処理時間: analysisResult.qualityMetrics?.processingTime + 'ms'
          });
          ...
          // 品質情報の取得
          const qualityInfo = analysisResult.qualityAssessment || { grade: 'C', issues: [] };
          ...
          // A級品質対応の信頼度レベルとカラー判定
          let confidenceLevel, confidenceColor, qualityBadge;
          if (confidence >= 0.8) {
            confidenceLevel = "最高";
          ...
          // 品質グレード情報の追加
          const gradeInfo = qualityInfo.grade === 'A' ? '🌟 最高品質分析達成!' : 
                           qualityInfo.grade === 'B' ? '✅ 高品質分析完了' : 
                           '📊 標準品質分析';
          ...
          if (hexagramCandidates && hexagramCandidates.length > 1) {
            const altList = hexagramCandidates.slice(1, 4).map(candidate => {
              ...
              if (altLine) {
                return `<li class="text-sm">🔮 ${altLine["卦名"] || altLine["親となる卦"]} (信頼度: ${Math.round(candidate.confidence * 100)}%) - ${candidate.reason}</li>`;
              }
              return null;
          ...
          const reasoning = `
            ...
            ご入力いただいた状況は、<strong>${line["卦名"] || line["親となる卦"]} ${line.爻}</strong>の「<strong>${mainTheme}</strong>」として分析されました。<br>
            <div class="analysis-metrics mt-2 p-2 bg-gray-900/50 rounded text-sm">
              ...
              <div class="text-gray-400">⏱️ 処理時間: ${Math.round(analysisResult.qualityMetrics.processingTime)}ms</div>
        ...
        window.submitFeedback = function(rating, analysisData) {
          try {
            ...
            const feedbackData = {
              ...
              hexagram: analysisData.startState?.['卦名'] || analysisData.startState?.['親となる卦'] || 'unknown',
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div class="analysis-comparison">
                ...
                <div class="text-sm text-gray-300">
                  ${currentAnalysisData.startState['卦名'] || currentAnalysisData.startState['親となる卦']} ${currentAnalysisData.startState.爻}<br>
              ...
              <div class="analysis-comparison">
                ...
                <div class="text-sm text-gray-300">
                  ${result.lineData['卦名'] || result.lineData['親となる卦']} ${result.lineData.爻}<br>
        ...
        console.log('✅ Module script fully loaded and ready');
