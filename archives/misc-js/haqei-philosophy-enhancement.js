...
class ContradictionAcceptanceSystem {
  ...
  justifyContradictions(contradictions) {
    return {
      philosophicalFramework: "HaQei分人思想では、一人の人間が複数の異なる、時には矛盾する側面を持つことは自然で健全なことです",
...
class EnhancedEightScenariosGenerator extends EightScenariosGenerator {
  constructor() {
    super();
    this.dynamicHaQei = new DynamicHaQeiSystem();
  ...
  async generatePhilosophicallyAlignedScenarios(iChingResult, userContext) {
    // 1. 動的分人生成
    const contextualPersonas = this.dynamicHaQei.generateContextualPersonas(
