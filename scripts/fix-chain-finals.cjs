...
// King Wen order minimal list (must match mock logic)
const MIN_HEX = [
...
function main() {
  ...
  let fixed = 0, touched = 0, nameFixed = 0;
  for (const key of Object.keys(db)) {
    const m = key.match(/^(.*?)\s+(初九|九二|九三|九四|九五|上六|上九|用九|用六)\s*\|\s*(JJJ|JJH|JHJ|JHH|HJJ|HJH|HHJ|HHH)$/);
    if (!m) continue;
    const startHex = m[1];
    const startPos = parsePos(m[2]);
    const pat = m[3];
    const expected = buildFinalFromPattern(startHex, startPos, pat);
    const v = db[key] || {};
    const f = v.final || {};
    const needHex = f.hex !== expected.hex;
    const needPos = Number(f.pos) !== Number(expected.pos);
    if (needHex || needPos) {
      v.final = { hex: expected.hex, pos: expected.pos };
      touched++;
    }
    if (f.name && /^(初九|九二|九三|九四|九五|上六|上九|用九|用六)$/.test(f.name)) {
      // remove ambiguous name label field to avoid混乱
      delete v.final.name; nameFixed++;
    }
    db[key] = v;
    if (needHex || needPos) fixed++;
  }
  fs.writeFileSync(file, JSON.stringify(db, null, 2), 'utf8');
  console.log('Finals fixed:', fixed, 'entries; removed name fields:', nameFixed);
}
