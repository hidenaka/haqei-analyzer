#!/usr/bin/env node
// Validate narrations for ALL 64×6×8 triad combinations using the same logic as UI
const fs = require('fs');
...
function loadH384H64() {
  ...
  const H64 = ctx.window.H64_DATA;
  const H384 = ctx.window.H384_DATA;
  if (!Array.isArray(H64) || !Array.isArray(H384)) throw new Error('Failed to load H64/H384');
  return { H64, H384 };
}
...
function pickKeyword(hex,line,overrides,states){
  ...
  if (arr.length){
    ...
    if (hit) return hit;
    // if not priority, try to use a token that looks normalized
    const known = arr.find(k=>k && !/[^\u3040-\u30ff\u4e00-\u9faf]/.test('')); // placeholder
    return arr[0];
  }
...
function main(){
  ...
  const banned = [/ですです/g,/ますです/g,/でします/g,/ありまします/g,/れります/g,/ながらながら/g,/厳格にです/g,/素晴/g,/（|）/g];
  let issues = [];
  let total = 0;
  for (const {hex,line,triad} of allCombos()){
    ...
    // Validate 3 stage narrations
    for (let i=0;i<3;i++){
      const s = narr[i];
      const errs = [];
      if (!/[。]$/.test(s)) errs.push('句点なし');
      if (!/(です|ます|ません)/.test(s)) errs.push('丁寧語なし');
      if (s.length>60) errs.push(`長すぎ(${s.length})`);
      for (const r of banned){ if (r.test(s)) errs.push(`禁止:${r}`); }
      if (errs.length){
        issues.push({ key:`${hex}-${line} ${triad} #${i+1}`, s, errs });
      }
    }
  }
  console.log(`Checked triad narrations: total combos=${total}`);
...
try{ main(); } catch(e){ console.error('validate-triad-narrations failed:', e.message); process.exit(1); }
