...
FutureSimulator.Core = {
  ...
  generateBinaryTreeHTML(result) {
    // BinaryTreeCompleteDisplayを呼び出し
    if (window.BinaryTreeCompleteDisplay) {
      setTimeout(() => {
        window.BinaryTreeCompleteDisplay.display(result);
      }, 100);
    }
    
    return `
      <div class="modern-analysis-results fade-in">
        <div id="binary-tree-results" class="binary-tree-container">
          <!-- BinaryTreeCompleteDisplayによって動的に生成される内容 -->
        </div>
      </div>
    `;
  },
