...
class RelationshipVisualizationEngine {
    constructor(internalCompatibilityEngine) {
        if (!internalCompatibilityEngine) {
            throw new Error("RelationshipVisualizationEngineにはInternalCompatibilityEngineインスタンスが必要です。");
        }
    ...
    analyzeRelationships(engineOsId, interfaceOsId, safeModeOsId) {
        ...
        const interactionData = [
            ...
            { source: 'Interface', target: 'SafeMode', compatibility: interfaceSafeMode },
        ];
    ...
    _generateBalanceChartData(compatibility) {
        ...
        const eiScore = compatibility.engineInterface.synergy - compatibility.engineInterface.conflict;
        const esScore = compatibility.engineSafeMode.synergy - compatibility.engineSafeMode.conflict;
        const isScore = compatibility.interfaceSafeMode.synergy - compatibility.interfaceSafeMode.conflict;
    ...
    _generateSwitchingPatterns(compatibility) {
        ...
        if (compatibility.engineInterface.type === CompatibilityType.CONFLICT) {
            patterns.push({
                ...
                description: "エンジンOSが主導権を握ろうとし、インターフェースOSが抵抗するパターンが見られます。セーフモードOSが過剰に反応し、内向的になる可能性があります。",
                advice: "互いの視点を理解し、共通の目標に焦点を当てることで、建設的な解決策を見出すことができます。"
            });
        }
        ...
        if (compatibility.engineSafeMode.type === CompatibilityType.TENSION) {
            patterns.push({
                ...
                description: "内的な不安や自己防衛が、本来のエンジンの力を抑制する可能性があります。",
                advice: "セーフモードOSのメッセージに耳を傾け、その根源にある感情を理解することで、より健全な自己表現が可能になります。"
            });
        }
        ...
        if (compatibility.overallAdvice.includes("大胆な挑戦")) {
            patterns.push({
                ...
                description: "エンジンOSが先導し、インターフェースOSが柔軟に対応、セーフモードOSが安定を保ち、全体としてスムーズに前進します。",
