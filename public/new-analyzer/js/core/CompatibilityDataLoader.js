...
class CompatibilityDataLoader {
    ...
    async getEngineInterfaceCompatibility(engineId, interfaceId) {
        try {
            ...
            // 指定されたインターフェースIDの組み合わせを検索
            const combination = fileData.internal_team_analysis.interface_combinations
                .find(combo => combo.interface_id === interfaceId);
                
            if (!combination) {
                throw new Error(`Compatibility data not found for Engine ${engineId} - Interface ${interfaceId}`);
            }
            ...
            const result = {
                ...
                engineName: fileData.internal_team_analysis.engine_os_name,
                interfaceName: combination.interface_name,
                type: combination.type,
                overallScore: combination.overall_score,
                summary: combination.summary,
                evaluation: combination.evaluation,
                advice: combination.advice,
                metadata: {
                    dataSource: 'engine-interface',
            ...
            console.log(`✅ Loaded compatibility: Engine ${engineId} - Interface ${interfaceId}`);
            return result;
            
        } catch (error) {
            console.error(`❌ Failed to get engine-interface compatibility:`, error);
            
            // フォールバック: 基本的な相性データを生成
            return this.generateFallbackCompatibility('engine-interface', engineId, interfaceId, error);
        }
    }
    ...
    async getEngineSafeModeCompatibility(engineId, safeModeId) {
        try {
            ...
            // 指定されたセーフモードIDの組み合わせを検索
            // engine-safemodeファイルも同じ構造の場合があるため、柔軟に対応
            const combinations = fileData.internal_team_analysis.safemode_combinations || 
                               fileData.internal_team_analysis.interface_combinations;
            
            if (!combinations) {
                throw new Error(`No combinations data found in engine-safemode file for Engine ${engineId}`);
            }
            
            const combination = combinations.find(combo => 
                (combo.safemode_id === safeModeId) || 
                (combo.interface_id === safeModeId) // フォールバック
            );
                
            if (!combination) {
                throw new Error(`Compatibility data not found for Engine ${engineId} - SafeMode ${safeModeId}`);
            }
            ...
            const result = {
                ...
                engineName: fileData.internal_team_analysis.engine_os_name,
                safeModeName: combination.safemode_name || combination.interface_name,
                type: combination.type,
                overallScore: combination.overall_score,
                summary: combination.summary,
                evaluation: combination.evaluation,
            ...
            console.log(`✅ Loaded compatibility: Engine ${engineId} - SafeMode ${safeModeId}`);
            return result;
            
        } catch (error) {
            console.error(`❌ Failed to get engine-safemode compatibility:`, error);
            
            // フォールバック: 基本的な相性データを生成
            return this.generateFallbackCompatibility('engine-safemode', engineId, safeModeId, error);
        }
    }
    ...
    async getInterfaceSafeModeCompatibility(interfaceId, safeModeId) {
        try {
            ...
            const estimation = await this.estimateInterfaceSafeModeCompatibility(interfaceId, safeModeId);
            ...
            console.log(`✅ Estimated compatibility: Interface ${interfaceId} - SafeMode ${safeModeId}`);
            return estimation;
            
        } catch (error) {
            console.error(`❌ Failed to get interface-safemode compatibility:`, error);
            
            // フォールバック: 基本的な相性データを生成
            return this.generateFallbackCompatibility('interface-safemode', interfaceId, safeModeId, error);
        }
    }
    ...
    async getTripleOSCompatibility(engineId, interfaceId, safeModeId) {
        try {
            ...
            // 並行して各相性データを取得
            const [engineInterface, engineSafeMode, interfaceSafeMode] = await Promise.all([
            ...
            console.log(`✅ Loaded triple OS compatibility: ${engineId}-${interfaceId}-${safeModeId}`);
            return result;
    ...
    async performJSONLoad(filePath, cacheKey) {
        ...
        for (let attempt = 1; attempt <= this.options.retryAttempts; attempt++) {
            try {
                ...
                lastError = error;
                console.warn(`⚠️ Failed to load ${filePath} (attempt ${attempt}):`, error.message);
        ...
        throw new Error(`Failed to load ${filePath} after ${this.options.retryAttempts} attempts: ${lastError.message}`);
    }
    ...
    async estimateInterfaceSafeModeCompatibility(interfaceId, safeModeId) {
        ...
        for (const engineId of sampleEngineIds) {
            try {
                ...
                console.warn(`⚠️ Failed to get estimation data for engine ${engineId}:`, error.message);
            }
        }
        ...
        // 推定アルゴリズム: 平均値ベース
        const avgScore = estimations.reduce((sum, est) => {
            return sum + (est.engineInterface.overallScore + est.engineSafeMode.overallScore) / 2;
        ...
        return {
            ...
            summary: `推定相性: ${estimatedType}タイプ (スコア: ${Math.round(avgScore * 100)}%)`,
     ...
     * 三重OS統合分析
     */
    analyzeTripleOSIntegration(engineInterface, engineSafeMode, interfaceSafeMode) {
        const scores = [
        ...
        return {
            ...
            dominantPattern: this.identifyDominantPattern([engineInterface, engineSafeMode, interfaceSafeMode]),
            integrationAdvice: this.generateIntegrationAdvice(engineInterface, engineSafeMode, interfaceSafeMode),
            riskFactors: this.identifyRiskFactors([engineInterface, engineSafeMode, interfaceSafeMode]),
            strengths: this.identifyStrengths([engineInterface, engineSafeMode, interfaceSafeMode])
        };
    }
    ...
    validateCompatibilityData(data, filePath) {
        ...
        if (!combinations || !Array.isArray(combinations)) {
            ...
            combinations.forEach((combo, index) => {
                ...
                if (!combo.type || !['SYNERGY', 'HARMONY', 'TENSION', 'CONFLICT', 'CHAOS'].includes(combo.type)) {
    ...
    generateFallbackCompatibility(type, id1, id2, error) {
        console.warn(`🔄 Generating fallback compatibility for ${type}: ${id1}-${id2}`);
        ...
        return {
            ...
            summary: `基本相性分析 (フォールバック): スコア ${Math.round(adjustedScore * 100)}%`,
            evaluation: this.generateEstimatedEvaluation(adjustedScore),
            advice: this.generateEstimatedAdvice(this.determineCompatibilityType(adjustedScore), adjustedScore),
            metadata: {
                dataSource: 'fallback',
    ...
    generateEstimatedAdvice(type, score) {
        const adviceMap = {
            'SYNERGY': { strengths: ['高い協調性'], challenges: ['過度の安定'], recommendations: ['新しい挑戦を取り入れる'] },
            'HARMONY': { strengths: ['バランス'], challenges: ['変化への対応'], recommendations: ['柔軟性を保つ'] },
            'TENSION': { strengths: ['創造的緊張'], challenges: ['葛藤管理'], recommendations: ['対話を重視する'] },
            'CONFLICT': { strengths: ['多様性'], challenges: ['対立解決'], recommendations: ['共通目標を設定する'] },
            'CHAOS': { strengths: ['変革力'], challenges: ['安定性'], recommendations: ['構造化を図る'] }
        };
    ...
    generateIntegrationAdvice(engineInterface, engineSafeMode, interfaceSafeMode) {
        return [
...
export default CompatibilityDataLoader;