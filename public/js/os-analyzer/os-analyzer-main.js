/**
 * OS Analyzer - Main Application Logic
 * T03: Extracted from inline script
 * HaQei Triple OS Analysis System
 */

'use strict';

console.log('OSAnalyzer version stamp A1');

const state = { tripleOSResults: null };

// runTripleOSAnalysis の早期定義（他ファイルからの呼び出しに対応）
function runTripleOSAnalysis(answers) {
    if (window.criticalCSSAnalyzer && typeof window.criticalCSSAnalyzer.runAnalysis === 'function') {
        return window.criticalCSSAnalyzer.runAnalysis(answers);
    }
    console.warn('⚠️ criticalCSSAnalyzer not available at runTripleOSAnalysis');
    return null;
}

function saveAnalysisResults(results){
  if(!results){ console.error('No results'); return; }
  state.tripleOSResults = results;
  try{ 
    localStorage.setItem('tripleOSResults', JSON.stringify(results));
    console.log('[OSAnalyzer] Results saved to localStorage');
  } catch(e) {
    console.error('[OSAnalyzer] Failed to save results to localStorage:', e);
    // エラーイベントを発火
    const errorEvent = new CustomEvent('haqei:tripleos:save-error', { detail: { error: e } });
    document.dispatchEvent(errorEvent);
  }
  updateDisplayWithResults(results);
}

function updateDisplayWithResults(results){
  const t0 = performance.now();
  
  // 各表示更新処理の個別パフォーマンス計測
  const t1 = performance.now();
  updateOSDisplay(results);
  console.log('[OSAnalyzer] updateOSDisplay(ms)=', Math.round(performance.now()-t1));
  
  const t2 = performance.now();
  updateHexagramDisplay(results.engine_os, results.interface_os, results.safe_mode_os);
  console.log('[OSAnalyzer] updateHexagramDisplay(ms)=', Math.round(performance.now()-t2));
  
  const t3 = performance.now();
  displayInteractions(results.interactions);
  console.log('[OSAnalyzer] displayInteractions(ms)=', Math.round(performance.now()-t3));
  
  // 全体の処理時間
  console.log('[OSAnalyzer] totalDisplay(ms)=', Math.round(performance.now()-t0));
}

function updateOSDisplay(results){
  const container = document.querySelector('#os-cards-container');
  if(!container){ console.warn('os-cards-container not found'); return; }
  container.innerHTML = `
    ...
    <div class="os-card interface">
      <h3>Interface OS</h3>
      <div class="os-strength">${results.interface_os.score.toFixed(2)}</div>
      <div class="os-hexagram">#${results.interface_os.id} ${results.interface_os.name}</div>
    </div>
    <div class="os-card safe">
      <h3>Safe Mode OS</h3>
      <div class="os-strength">${results.safe_mode_os.score.toFixed(2)}</div>
      <div class="os-hexagram">#${results.safe_mode_os.id} ${results.safe_mode_os.name}</div>
    </div>
  `;
}

function updateHexagramDisplay(engine, inter, safe){
  const el = document.querySelector('#hexagram-display');
  if(!el){ return; }
  el.innerHTML = `#${engine.id} ${engine.name} / #${inter.id} ${inter.name} / #${safe.id} ${safe.name}`;
}

function displayInteractions(ints){
  const el = document.querySelector('#summary-view');
  if(!el){ return; }
  el.textContent = (ints && ints.summary) || fallbackExpressions.processing;
}

// グローバル公開（早期）
window.runTripleOSAnalysis = window.runTripleOSAnalysis || runTripleOSAnalysis;
window.saveAnalysisResults = window.saveAnalysisResults || saveAnalysisResults;
window.updateDisplayWithResults = window.updateDisplayWithResults || updateDisplayWithResults;
window.updateOSDisplay = window.updateOSDisplay || updateOSDisplay;
window.updateHexagramDisplay = window.updateHexagramDisplay || updateHexagramDisplay;
window.displayInteractions = window.displayInteractions || displayInteractions;
        ...
        // 2. 質問数確認と検証
        console.log(`✅ HaQei 36問質問システム構築完了: ${QUESTIONS.length}問`);
        ...
        // 初期化時に検証実行
        const isSystemValid = validateQuestionSystem();
        if (!isSystemValid) {
        ...
        class TripleOSEngine {
            ...
            async buildOSResult(trigramVector, osType) {
                ...
                // 緊急修正: hexagramNameの確実な設定
                const guaranteedHexagramName = hexagramData.name_jp || hexagramData.name || this.getHexagramName(validHexagramId);
                
                console.log(`🔧 [${osType}] OS結果構築:`, {
                    hexagramId: validHexagramId,
                    hexagramName: guaranteedHexagramName,
                    hexagramDataExists: !!hexagramData,
                    hexagramDataName: hexagramData.name_jp || hexagramData.name
                });
        ...
        class CriticalCSSAnalyzer {
            ...
            bindEvents() {
                // ナビゲーション（nullガード付き + 重複バインド防止）
                const startBtn = document.getElementById('start-btn');
                if (startBtn && !startBtn.hasAttribute('data-critical-handler-bound')) {
                    startBtn.addEventListener('click', () => this.startAnalysis());
                    startBtn.setAttribute('data-critical-handler-bound', 'true');
                    console.log('✅ Start button event handler attached (CriticalCSSAnalyzer)');
                } else if (startBtn) {
                    console.log('ℹ️ Start button handler already bound, skipping (CriticalCSSAnalyzer)');
                }
                
                // QuestionManagerが次へ・前へボタンを管理するため、重複登録を削除
            ...
            showScreen(screenId) {
                ...
                if (targetScreen && targetScreen.classList) {
                    ...
                    console.error(`❌ CriticalCSSAnalyzer: Screen element not found: ${screenId}`);
                }
            }
            ...
            showQuestion(index) {
                ...
                if (progressFill && progressFill.style) {
                    ...
                    console.warn('⚠️ CriticalCSSAnalyzer: progress-fill element not found');
                }
                
                // QuestionManagerに委譲
                if (window.QuestionManager && window.QuestionManager.displayQuestion) {
                    // QuestionManagerのdisplayQuestionを使用
                    window.QuestionManager.displayQuestion(question, index);
                } else {
                    console.error('❌ CriticalCSSAnalyzer: QuestionManager not available, falling back to original implementation');
                    ...
                    if (questionNumber) {
                        ...
                        console.warn('⚠️ CriticalCSSAnalyzer: question-number element not found');
                    }
                    
                    if (questionTitle) {
                        questionTitle.textContent = question.text;
                    } else {
                        console.warn('⚠️ CriticalCSSAnalyzer: question-title element not found');
                    }
                    ...
                    if (container) {
                        ...
                        console.error('❌ CriticalCSSAnalyzer: options-container element not found');
                        return;
                    }
            ...
            async showResults(tripleOSResults) {
                try {
                    ...
                    document.getElementById('summary-view').innerHTML = summaryHTML;
            ...
            createEnhancedOSCard(osName, osData, color) {
                ...
                // 64卦IDと卦名を取得（緊急修正: 確実な設定）
                const hexagramId = osData.hexagramId || 1;
                const hexagramName = osData.hexagramName || this.getHexagramName(hexagramId);
                
                console.log(`🎨 [${osName}] OSカード生成:`, {
                    hexagramId,
                    hexagramName,
                    osDataHexagramName: osData.hexagramName
                });
                ...
                try {
                    ...
                    if (ariaLive && ariaLive.textContent !== undefined) {
                        try {
                            ...
                            console.error('❌ announce: textContent設定エラー:', textError);
                            this.createFallbackAnnouncement(message);
                        }
                    } else {
                        console.warn('⚠️ announce: aria-live要素が見つからないか無効です');
                        this.createFallbackAnnouncement(message);
                    }
                } catch (error) {
                    console.error('❌ announce: 全体エラー:', error);
                    // 最終フォールバック
                    console.log('📢 Announcement:', message);
                }
            }
            
            createFallbackAnnouncement(message) {
                try {
                    // フォールバック：要素を動的に作成
                    const fallback = document.createElement('div');
                    fallback.id = 'aria-live-region-fallback';
                    ...
                    console.error('❌ createFallbackAnnouncement エラー:', fallbackError);
                    console.log('📢 Final Fallback Announcement:', message);
                }
            }
