...
class SituationalHexagramTester {
  ...
  async initializeEngines() {
    ...
    try {
      ...
      if (typeof ProbabilisticSituationModeler !== 'undefined') {
        this.engines.probabilisticModeler = new ProbabilisticSituationModeler();
        console.log('✅ ProbabilisticSituationModeler 初期化完了');
      }
  ...
  async runSingleTest(testUser, worryText, options = {}) {
    ...
    try {
      ...
      // Phase 4: 確率的モデリング（オプション）
      let probabilisticResult = null;
      if (this.engines.probabilisticModeler && options.includeProbabilistic) {
        probabilisticResult = await this.runProbabilisticModeling(
          situationalResult,
          hexagramResult
        );
      }
   ...
   * Phase 4: 確率的モデリング（オプション）
   */
  async runProbabilisticModeling(situationalResult, hexagramResult) {
    if (!this.engines.probabilisticModeler) {
      return null;
    }
    
    return await this.engines.probabilisticModeler.modelProbabilisticSituation(
      situationalResult,
      hexagramResult
    );
  }
  ...
  getEngineVersions() {
    return {
      ...
      hexagramMapper: '2.5.0',
      psychologicalAnalyzer: '3.0.0',
      probabilisticModeler: '4.0.0'
    };
  }
