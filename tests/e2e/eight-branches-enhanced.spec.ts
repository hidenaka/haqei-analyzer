...
test.describe('EightBranches Enhanced UI', () => {
  test('renders 8 cards with enhanced elements (JP)', async ({ page }) => {
    ...
    await expect(root.locator('text=選ぶ理由')).toBeVisible({ timeout: 20000 });
    await expect(root.locator('text=次の一手')).toBeVisible({ timeout: 20000 });
    await expect(root.locator('text=特徴(卦・爻)')).toBeVisible({ timeout: 20000 });

    // 意思決定支援
    await expect(root.locator('text=合う条件')).toBeVisible();
    await expect(root.locator('text=注意点')).toBeVisible();
    await expect(root.locator('text=成果イメージ')).toBeVisible();
    ...
    await expect(
      influence.or(influencePct).or(confidence)
    ).toBeVisible();
